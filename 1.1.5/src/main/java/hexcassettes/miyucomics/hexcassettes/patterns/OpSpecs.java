...
public class OpSpecs implements SpellAction {
    ...
    public SpellAction.@NotNull Result executeWithUserdata(
        ...
        try {
            if (!(args.get(0) instanceof ListIota list)) {
                env.getWorld().playSound(null, player.blockPosition(), ModSounds.CASSETTE_FAIL.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
                System.out.println("[Hexcassettes] OpSpecs: arg 0 is not a list!");
                return noOp();
            }
            if (!(args.get(1) instanceof DoubleIota delayIota)) {
                env.getWorld().playSound(null, player.blockPosition(), ModSounds.CASSETTE_FAIL.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
                System.out.println("[Hexcassettes] OpSpecs: arg 1 is not a number!");
                return noOp();
            }
            ...
            CompoundTag hexNbt = IotaType.serialize(list);
            ...
            System.out.println("[Hexcassettes] Recorded spell to edit slot " + state.editSlot);
            env.getWorld().playSound(null, player.blockPosition(), ModSounds.CASSETTE_INSERT.get(), SoundSource.PLAYERS, 1.0f, 1.0f);

        } catch (Exception e) {
            env.getWorld().playSound(null, player.blockPosition(), ModSounds.CASSETTE_FAIL.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
            System.out.println("[Hexcassettes] OpSpecs FAILED: " + e);
            e.printStackTrace();
        }
